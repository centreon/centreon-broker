...
#define TEST_NAME "rtmonitoring_v2_servicedependencies"
...
/**
 *  Check that the service dependencies table work properly.
 *
...
int main() {
  ...
  try {
    ...
    cbmod_cfg.set("TCP_PORT", "5580");
    test::centengine_config engine_config;
    engine_config.generate_hosts(10);
    engine_config.generate_services(1);
    engine_config.service_depends_on("1", "1", "2", "2");
    engine_config.service_depends_on("4", "4", "5", "5");
