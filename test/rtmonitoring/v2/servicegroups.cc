...
int main() {
  ...
  try {
    // Database.
    char const* tables[] = {
      "instances", "hosts", "services",
      "servicegroups", "services_servicegroups", NULL
    };
    ...
    for (int i(0); i < 5; ++i) {
      test::centengine_object
        group(test::centengine_object::servicegroup_type);
      {
    ...
    test::centengine_object&
      hg(engine_config.get_service_groups().front());
