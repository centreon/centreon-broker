...
class state {
 public:
  ...
  void event_queue_max_size(int val) throw();
  ...
  void log_human_readable_timestamp(bool human_log_time) throw();
  ...
  std::map<std::string, std::string>& params() throw();
  std::map<std::string, std::string> const& params() const throw();
