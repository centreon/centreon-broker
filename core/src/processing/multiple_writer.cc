  ...
  for (std::list<misc::shared_ptr<io::stream> >::iterator
    ...
    try {
      ...
      logging::debug(logging::medium)
        << "failover: normal termination of a secondary failover endpoint";
      _secondary_outputs.erase(it);
    }
    catch (std::exception const& e) {
      logging::error(logging::medium)
        << "failover: a secondary endpoint failed: '" << e.what()
        << "', closing it.";
      _secondary_outputs.erase(it);
    }
  }
...
void multiple_writer::register_secondary_endpoints(
    ...
    for (std::vector<misc::shared_ptr<io::endpoint> >::iterator
         ...
         ++it, ++num) {
      logging::debug(logging::medium)
        << "failover: endpoint opening secondary failover number " << num;
