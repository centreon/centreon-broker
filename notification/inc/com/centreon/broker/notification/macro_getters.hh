 ...
 */
template <>
void inline set_precision<0>(std::ostringstream& ost) {
  (void)ost;
}
 ...
 */
template <>
std::string inline to_string<std::string, 0>(std::string const& value) {
  return (value);
}
...
std::string get_host_member_as_string(macro_context const& context) {
  return (to_string<U, precision>(
      context.get_cache().get_host(context.get_id()).get_node().*member));
}
 ...
 */
template <typename T, typename U, U(T::*member), int precision>
std::string get_service_member_as_string(macro_context const& context) {
  return (to_string<U, precision>(
      context.get_cache().get_service(context.get_id()).get_node().*member));
}
 ...
 */
template <typename T, typename U, U(T::*member), int precision>
std::string get_host_status_member_as_string(macro_context const& context) {
  return (to_string<U, precision>(
      context.get_cache().get_host(context.get_id()).get_status().*member));
}
 ...
 */
template <typename T, typename U, U(T::*member), int precision>
std::string get_service_status_member_as_string(macro_context const& context) {
  return (to_string<U, precision>(
      context.get_cache().get_service(context.get_id()).get_status().*member));
}
 ...
 */
template <typename T, typename U, U(T::*member), int precision>
std::string get_host_prevstatus_member_as_string(macro_context const& context) {
  return (to_string<U, precision>(
      context.get_cache().get_host(context.get_id()).get_prev_status().*
      member));
}
...
std::string get_service_prevstatus_member_as_string(
  ...
  return (to_string<U, precision>(
      context.get_cache().get_service(context.get_id()).get_prev_status().*
      member));
}
 ...
 *
 *  @tparam get_long_output  True if we want the long output, false if we want
 * the short one.
 *
 *  @param[in] context       The context from where the macro is being executed.
 *
...
template <>
std::string get_host_output<false>(macro_context const& context);

template <>
std::string get_host_output<true>(macro_context const& context);
 ...
 */
template <bool get_long_output>
std::string get_service_output(macro_context const& context
                               __attribute__((unused))) {
  return "";
}

template <>
std::string get_service_output<false>(macro_context const& context);

template <>
std::string get_service_output<true>(macro_context const& context);
...
std::string get_datetime_string(macro_context const& context) {
  return (utilities::get_datetime_string(
      ::time(NULL), 48, date_type, context.get_state().get_date_format()));
}

/**
 *  Get the total number of services associated with the host.
 *
 *  @tparam service_state  The state of the services we want to count: -1 for
 * all.
 *
 *  @param[in] context     The context from where the macro is being executed.
 *
...
std::string get_total_host_services(macro_context const& context) {
 ...
 *
 *  @tparam host_state  The state of the hosts we want to count. -1 for all that
 * are not up.
 *
 *  @param[in] context  The context from where the macro is being executed.
 *
...
std::string get_total_hosts(macro_context const& context) {
 ...
 */
template <int service_state>
std::string get_total_services(macro_context const& context) {
  return (to_string<int, 0>(
 ...
 */
template <int host_state>
std::string get_total_hosts_unhandled(macro_context const& context) {
  size_t count = 0;
 ...
 */
template <int service_state>
std::string get_total_services_unhandled(macro_context const& context) {
  size_t count = 0;
  node_cache const& cache = context.get_cache();
  state const& st = context.get_state();
...
std::string get_contact_member(macro_context const& context) {
  return (to_string<typename remove_all<U>::type, precision>(
      (context.get_contact().*member)()));
}
...
std::string get_action_member(macro_context const& context) {
  return (to_string<typename remove_all<U>::type, precision>(
      (context.get_action().*member)()));
}
 ...
 */
template <int number>
std::string get_address_of_contact(macro_context const& context) {
  std::stringstream ss;
  ss << "address" << number;
  return (context.get_state()
              .get_contact_infos(context.get_contact().get_id())
              .value(ss.str()));
}

// Static, non template getters.
std::string get_host_state(macro_context const& context);

std::string get_service_state(macro_context const& context);

std::string get_last_host_state(macro_context const& context);

std::string get_last_service_state(macro_context const& context);

std::string get_host_state_type(macro_context const& context);

std::string get_service_state_type(macro_context const& context);

std::string null_getter(macro_context const& context);

std::string get_host_duration(macro_context const& context);

std::string get_service_duration(macro_context const& context);

std::string get_host_duration_sec(macro_context const& context);

std::string get_service_duration_sec(macro_context const& context);

std::string get_node_downtime_number(macro_context const& context);

std::string get_timet_string(macro_context const& context);

std::string get_notification_type(macro_context const& context);
}  // namespace notification
