 ...
 * @param[out] output   A dependency builder object to register the
 * dependencies.
 */
void dependency_loader::load(mysql* ms, dependency_builder* output) {
  ...
  try {
    ...
    throw exceptions::msg()
        << "notification: cannot load dependencies from database: " << e.what();
  }

  // Get the relations of the dependencies. It will also give us their type.
  _load_relations(ms, *output);

  // We now know the types of the dependencies: we can parse the failure
  // options.
 ...
 */
void dependency_loader::_load_relations(mysql* ms, dependency_builder& output) {
  std::promise<database::mysql_result> promise;
  ms->run_query_and_get_result(
  ...
  try {
    database::mysql_result res(promise.get_future().get());
    while (ms->fetch_row(res))
      output.dependency_node_id_child_relation(res.value_as_u32(0),
  ...
  try {
    database::mysql_result res(promise.get_future().get());
    while (ms->fetch_row(res))
      output.dependency_node_id_parent_relation(res.value_as_u32(0),
                                                node_id(res.value_as_u32(1)));
  } catch (std::exception const& e) {
    throw exceptions::msg()
  ...
  try {
    ...
    while (ms->fetch_row(res))
      output.dependency_node_id_child_relation(
          res.value_as_u32(0),
          node_id(res.value_as_u32(2), res.value_as_u32(1)));
  } catch (std::exception const& e) {
    throw exceptions::msg() << "notification: cannot load service/child "
...
/**
 *  Load a relation between two ids, one of which is dependency_dep_id, from a
 ...
 *  @param[in] register_method          The dependency builder method to call to
