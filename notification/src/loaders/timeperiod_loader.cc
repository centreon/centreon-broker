 ...
 *  @param[out] output   A timeperiod builder object to register the
 * timeperiods.
 */
void timeperiod_loader::load(mysql* ms, timeperiod_builder* output) {
  ...
  try {
    ...
    while (ms->fetch_row(res)) {
      ...
      output->add_timeperiod(timeperiod_id, tperiod);
    }
  } catch (std::exception const& e) {
    throw exceptions::msg()
        << "notification: cannot load timeperiods from database: " << e.what();
  }
  ...
  try {
    database::mysql_result res(promise.get_future().get());
    while (ms->fetch_row(res))
      output->add_timeperiod_exclude_relation(res.value_as_u32(1),
                                              res.value_as_u32(0));
  } catch (std::exception const& e) {
    throw exceptions::msg()
  ...
  try {
    database::mysql_result res(promise.get_future().get());
    while (ms->fetch_row(res))
      output->add_timeperiod_include_relation(res.value_as_u32(1),
                                              res.value_as_u32(0));
  } catch (std::exception const& e) {
    throw exceptions::msg()
