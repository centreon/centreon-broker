...
class node : public correlation::state {
 public:
  ...
  void manage_status(short status,
                     ...
                     io::stream* stream);
  void manage_ack(neb::acknowledgement const& ack, io::stream* stream);
  void manage_downtime(neb::downtime const& dwn, io::stream* stream);
  void manage_log(neb::log_entry const& entry, io::stream* stream);
  enum link_type { parent, children, depended_by, depends_on };
  void linked_node_updated(node& n,
 ...
 private:
  ...
  void _generate_state_event(timestamp start_time,
                             short new_status,
                             bool new_in_downtime,
                             io::stream* stream);
  correlation::state _open_state_event(timestamp start_time) const;

  void _visit_linked_nodes(timestamp last_state_change,
                           bool closed,
                           io::stream* stream);
  void _visit_parent_of_child_nodes(timestamp last_state_change,
                                    bool closed,
                                    io::stream* stream);
