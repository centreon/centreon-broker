...
void influxdb12::commit() {
  ...
  if (ec)
    throw exceptions::msg()
        << "influxdb: unable to get remote endpoint from socket: "
        << ec.message();
  ...
  if (err)
    throw exceptions::msg()
        << "influxdb: couldn't commit data to InfluxDB with address '" << addr
        << "' and port '" << port << "': " << err.message();
  ...
  do {
    ...
    if (err)
      throw exceptions::msg()
          << "influxdb: couldn't receive InfluxDB answer with address '" << addr
          << "' and port '" << port << "': " << err.message();
...
bool influxdb12::_check_answer_string(std::string const& ans,
  ...
  if (split.size() < 3)
    throw exceptions::msg()
        << "influxdb: unrecognizable HTTP header for '" << addr
        << "' and port '" << port << "': got '" << first_line_str << "'";
  ...
  if (split[0] == "HTTP/1.0" && split[1] == "204" && split[2] == "No" &&
    ...
    throw exceptions::msg() << "influxdb: got an error from '" << addr
                            << "' and port '" << port << "': '" << ans << "'";
}
