...
function init(conf)
  ...
  simu.conn = {}
  simu.conn["storage"] = env:connect(conf['database_storage'], conf['login'], conf['password'], conf['db_addr'], 3306)
  ...
  simu.conn["cfg"] = env:connect(conf['database_conf'], conf['login'], conf['password'], conf['db_addr'], 3306)
...
function read()
  if not simu.infinite then
    ...
    if simu.step_check < simu.step_build or not step[simu.step_check].count.continue then
      ...
      if cont and simu.step_check > #step then
        simu.infinite = true
        simu.step_build = 1
        cont = false
        for i,v in ipairs(step) do
          if v.count.rep then
            cont = true
            print(red .. "Entering into the infinite loop" .. reset)
            break
          end
        end
      end
    ...
    if not simu.last_loop or os.time() >= simu.last_loop + simu.delay then
      -- Case of infinite loop
      print(green .. "GO!!!" .. reset)
      if simu.step_build > #step then
        simu.step_build = 1
      end
      while not step[simu.step_build].count.rep do
        simu.step_build = simu.step_build + 1
        if simu.step_build > #step then
          simu.step_build = 1
        end
      ...
      if step[simu.step_build] then
        ...
        simu.step_build = simu.step_build + 1
        if simu.step_build > #step then
          simu.last_loop = os.time()
          print(blue .. "Stack contains " .. #simu.stack .. " events")
        end
      end
    end
  end

  -- Need to pop elements from the stack
