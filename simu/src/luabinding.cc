...
bool luabinding::_parse_event(std::shared_ptr<io::data>& d) {
  ...
  while (lua_next(_L, -2) != 0) {  // -2, because we have table at -1
    if (lua_isstring(_L, -2)) {    // only store stuff with string keys
      ...
      if (lua_isboolean(_L, -1))
        ...
        map[key] = QVariant(static_cast<int64_t>(lua_tointeger(_L, -1)));
        ...
        if (valued - valuei < eps && valuei - valued < eps)
          map[key] = QVariant::fromValue(vi);
        else
          map[key] = QVariant::fromValue(valued);
      }
#endif
  ...
  io::event_info const* info(
      io::events::instance().get_event_info(map["_type"].toULongLong()));
  if (info) {
    // Create event
    ...
    if (t) {
        ...
        if (it != map.end()) {
          ...
          switch (current_entry->get_type()) {
            case mapping::source::BOOL:
              current_entry->set_bool(*t, it.value().toBool());
              break;
            case mapping::source::DOUBLE:
              current_entry->set_double(*t, it.value().toDouble());
              break;
            case mapping::source::INT:
              current_entry->set_int(*t, it.value().toInt());
              break;
            case mapping::source::SHORT:
              current_entry->set_short(*t,
                                       static_cast<short>(it.value().toInt()));
              break;
            case mapping::source::STRING:
              current_entry->set_string(*t, it.value().toString());
              break;
            case mapping::source::TIME:
              current_entry->set_time(*t, it.value().toLongLong());
              break;
            case mapping::source::UINT:
              current_entry->set_uint(*t, it.value().toULongLong());
              break;
            default:
      ...
      throw exceptions::msg()
          << "simu: cannot create object of ID " << map["_type"].toInt()
          << " whereas it has been registered";
  } else {
    logging::info(logging::high)
        << "simu: cannot unserialize event of ID " << map["_type"].toUInt()
