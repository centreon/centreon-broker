 ...
 */
void parser::parse_perfdata(uint32_t host_id, uint32_t service_id,
  ...
  while (*tmp) {
    ...
    if (end - s + 1 > 0) {
      ...
      log_v2::perfdata()->error("In service {}, metric name empty before '{}...'",
                                id(), fmt::string_view(s, 10));
      error = true;
    }
    ...
    if (*tmp != '=') {
      ...
      log_v2::perfdata()->error(
          "invalid perfdata format in service {}: equal sign not present or misplaced '{}'", id(),
          fmt::string_view(s, (tmp - s) + i));
      error = true;
    } else
      ++tmp;
    ...
    if (std::isnan(p.value())) {
      ...
      log_v2::perfdata()->error(
          "storage: invalid perfdata format in service {}: no numeric value after equal sign "
          "'{}'", id(),
          fmt::string_view(s, (tmp - s) + i));
      error = true;
      tmp = skip(tmp);
    ...
    log_v2::perfdata()->debug(
        ...
        "critical={}, min={}, max={})", p.name(), p.value(), p.unit(),
        p.warning(), p.critical(), p.min(), p.max());
