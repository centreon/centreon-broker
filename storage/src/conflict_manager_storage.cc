...
void conflict_manager::_update_metrics() {
  ...
  for (auto it = _metrics.begin(); it != _metrics.end(); ++it) {
    ...
    m.emplace_back(fmt::format(
        ...
        std::isnan(metric->warn) || std::isinf(metric->warn) ? "NULL" : fmt::format("{}", metric->warn),
        std::isnan(metric->warn_low) || std::isinf(metric->warn_low) ? "NULL"
                                     : fmt::format("{}", metric->warn_low),
        metric->warn_mode ? "1" : "0",
        std::isnan(metric->crit) || std::isinf(metric->crit) ? "NULL" : fmt::format("{}", metric->crit),
        std::isnan(metric->crit_low) || std::isinf(metric->crit_low) ? "NULL"
