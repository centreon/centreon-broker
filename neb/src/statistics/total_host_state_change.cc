 ...
 *  @return This object.
 */
total_host_state_change& total_host_state_change::operator=(
    total_host_state_change const& right) {
 ...
 */
void total_host_state_change::run(std::string& output, std::string& perfdata) {
  if (!com::centreon::engine::host::hosts.empty()) {
    ...
    oss << "Engine " << config::applier::state::instance().poller_name()
        << " has an average host state change of " << std::fixed
