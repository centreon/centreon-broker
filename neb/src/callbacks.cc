...
int neb::callback_host(int callback_type, void* data) {
  ...
  try {
    ...
    my_host->notify_on_flapping = h->get_notify_on(engine::notifier::flapping);
    ...
    if (host_id != 0) {
      ...
      for (std::pair<std::string, engine::customvariable> const& p : h->custom_variables) {
        engine::customvariable const& cvar{p.second};
        std::string const& name{p.first};
        if (cvar.is_sent() && name != "HOST_ID") {
          ...
          data.var_value = const_cast<char*>(cvar.get_value().c_str());
...
int neb::callback_service(int callback_type, void* data) {
  ...
  try {
    ...
    my_service->notify_on_flapping = s->get_notify_on(engine::notifier::flapping);
    ...
    if (my_service->host_id && my_service->service_id) {
      ...
      // Generate existing custom variables.
      for (std::unordered_map<std::string, engine::customvariable>::const_iterator
